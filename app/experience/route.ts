...
<style>
  ...
  .sx-context{margin:0 0 12px;padding:9px 11px;border:1px solid #d7e3ef;border-radius:9px;background:#f7fafc;font:600 11px Inter,Arial,sans-serif;color:#50657b}.sx-context b{color:#0b2d5e}.sx-context .sx-context-events{margin-top:6px;display:flex;gap:12px;flex-wrap:wrap;font-size:10px;font-weight:500}.sx-context .sx-context-events span{color:#6b7b8f}
</style>
<script>
  (() => {
    ...
    const presenterPhase = {documents:0,exception:1,prioritized:2,assigned:3,scheduled:3,gate:4,yard:4,delivered:5,"empty-return":5,closed:6};
    const contextTitles = {documents:"Documents",exception:"Exceptions",prioritized:"Prioritization",assigned:"Dispatch",scheduled:"Dock Calendar",gate:"Gate Management",yard:"Yard Map",delivered:"Client Delivery","empty-return":"Empty Returns",closed:"Cycle Performance"};
    const renderContext = () => {
      const s=getSession(), latest=s.events[s.events.length-1], content=document.getElementById('content'); if(!content) return;
      let banner=document.getElementById('sx-context'); if(!banner){ banner=document.createElement('div'); banner.id='sx-context'; banner.className='sx-context'; content.prepend(banner); }
      banner.innerHTML='<b>Demo session · '+contextTitles[s.phase]+'</b> · '+s.container+' · '+latest.status+'<div class="sx-context-events"><span>'+latest.at+' · '+latest.actor+'</span><span>Source: '+latest.source+' · simulated</span><span>Next: '+latest.nextAction+'</span></div>';
    };
    const updateMobileHandoff = phase => {
      if(typeof mobileStage==='undefined') return;
      const map={documents:'port-out',exception:'port-out',prioritized:'port-out',assigned:'port-out',scheduled:'port-out',gate:'gate-in',yard:'yard-arrival',delivered:'delivery', 'empty-return':'empty-return',closed:'delivery'};
      mobileStage=map[phase]||mobileStage; portDeparted=['gate','yard','delivered','empty-return','closed'].includes(phase); yardArrived=['yard','delivered','empty-return','closed'].includes(phase); deliveryClosed=['delivered','empty-return','closed'].includes(phase); driverLocation=phase==='yard'?'yard':phase==='delivered'||phase==='empty-return'||phase==='closed'?'client':'port';
      if(typeof operatorJobStep!=='undefined'){ operatorJobStep=phase==='yard'?'picked':phase==='delivered'||phase==='empty-return'||phase==='closed'?'dropped':'assigned'; operatorContainer='TCKU5519902'; }
      const mobileOverlay=document.getElementById('mobile-overlay'); if(typeof renderMobile==='function' && mobileRole && mobileOverlay) mobileOverlay.innerHTML=renderMobile();
    };
    const renderSession = () => {
    ...
    const advanceSession = () => {
      ...
      updateMobileHandoff(updated.phase); if (typeof renderAll==='function') renderAll(); save(); renderSession(); renderContext();
    ...
    const presenter=document.createElement('div'); presenter.id='sx-presenter'; presenter.className='sx-presenter'; presenter.innerHTML='<div class="sx-presenter-card"><div style="display:flex;justify-content:space-between"><div><h2>Presenter Mode · Celsur Operations</h2><small>20-minute guided story · Powered by ShipmentX YMS</small></div><div style="text-align:right"><b class="sx-presenter-timer">00:00</b><br><small class="sx-presenter-cue">Opening scenario</small></div></div><div class="agenda">'+['Operating day and exposure','Resolve customs blocker','Priority and dispatch eligibility','Assign and schedule movement','Yard and driver execution','Delivery, POD and empty return','Outcome and measurable impact'].map((x,i)=>'<div class="agenda-num">'+(i+1)+'</div><div class="agenda-item">'+x+'</div>').join('')+'</div><div class="sx-presenter-actions"><button class="sx-presenter-reset">Reset opening scenario</button><button class="secondary sx-presenter-close">Return to app</button></div></div>';
    document.body.appendChild(presenter); presenter.querySelector('.sx-presenter-close').addEventListener('click',()=>presenter.classList.remove('open')); presenter.querySelector('.sx-presenter-reset').addEventListener('click',()=>{localStorage.removeItem(key);localStorage.removeItem(sessionKey);localStorage.removeItem('shipmentx-presenter-start');window.location.reload();});
    const presenterStart=()=>{if(!localStorage.getItem('shipmentx-presenter-start'))localStorage.setItem('shipmentx-presenter-start',String(Date.now())); presenter.classList.add('open');};
    const updatePresenter=()=>{const s=getSession(),start=Number(localStorage.getItem('shipmentx-presenter-start')||Date.now()),elapsed=Math.max(0,Math.floor((Date.now()-start)/1000)); presenter.querySelector('.sx-presenter-timer').textContent=String(Math.floor(elapsed/60)).padStart(2,'0')+':'+String(elapsed%60).padStart(2,'0'); const active=presenterPhase[s.phase]||0; presenter.querySelectorAll('.agenda-item').forEach((el,i)=>el.classList.toggle('active',i===active)); presenter.querySelector('.sx-presenter-cue').textContent=['Establish the operating day and exposure','Resolve the golden container blocker','Show the eligibility change','Assign and schedule the movement','Execute yard and driver workflow','Complete POD and empty return','Close on measurable impact'][active];};
    setInterval(updatePresenter,1000); updatePresenter();
    const presenterEnabled=new URLSearchParams(window.parent===window?location.search:window.parent.location.search).get('presenter')==='1'; if(presenterEnabled){const p=document.createElement('button');p.className='demo-reset';p.textContent='Presenter';p.addEventListener('click',presenterStart);(document.querySelector('.tb-actions')||document.body).appendChild(p);}
    window.addEventListener('keydown',e=>{if(e.ctrlKey&&e.shiftKey&&e.key.toLowerCase()==='p')presenter.classList.contains('open')?presenter.classList.remove('open'):presenterStart();});
    renderSession(); renderContext();
